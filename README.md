...
| **数据存储** | **EdgeOne Pages KV**（键值存储） | EdgeOne |

> 前端所有页面都是客户端组件，没有任何 SSR / 服务端取数依赖。
> 后端密码哈希与会话签名全部用 Web Crypto，能跑在 Edge 运行时。

存储键名（扁平、可按前缀遍历，匹配 EdgeOne KV 的 `list(prefix)`）：
...
edgeone/lib/router.js       ← 后端核心逻辑（KV + 密码 + 会话 + 路由）
```

部署只需 3 步配置：

1. **KV 存储**：在 EdgeOne 控制台创建 KV 命名空间，绑定到项目，
   **变量名填 `DB`**（入口文件 `functions/api/[[path]].js` 认这个名字）。
...
部署后，前端页面走 EdgeOne 静态托管，`/api/*` 自动走 Pages Function 读写 EdgeOne KV，
...
edgeone/
  lib/router.js            # ★ EdgeOne 生产后端核心（KV + 密码 + 会话 + 路由）
